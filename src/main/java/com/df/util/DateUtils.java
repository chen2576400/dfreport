...
public class DateUtils {
     ...
     * @return
     */
    public static Date getStartTime(Long timestamp) {
        LocalDateTime now;
        if (timestamp==null){
            now = LocalDateTime.now().withHour(0).withMinute(0).withSecond(0);
        }
        now=localDateTime(timestamp).withHour(0).withMinute(0).withSecond(0);
    ...
    public static Date getEndTime(Long timestamp) {
        ...
        now=localDateTime(timestamp).withHour(23).withMinute(59).withSecond(59).withNano(999);
        return localDateTime2Date(now);
    }
